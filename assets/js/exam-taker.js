    ...
    currentExam.sections.forEach((sec, sIdx) => {
        ...
        if (sec.type === 'gap_fill') {
            ...
            let contentText = sec.content.replace(/\{([^}]+)\}/g, (match, gapId) => {
                ...
                return `
                    <select class="inline-block border-b-2 border-blue-500 font-bold text-blue-800 mx-1 bg-blue-50 py-1 px-2 rounded cursor-pointer" 
        ...
        // 2. READING
            ...
            let readingText = sec.content ? sec.content : "(Bài đọc đang trống)";
            readingText = readingText.replace(/\n/g, '<br>');

            contentHtml = `
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[500px]">
                    <div class="bg-gray-50 p-6 rounded-lg border overflow-y-auto leading-relaxed text-justify font-serif text-gray-800 text-base shadow-inner">
                        ${readingText}
                    </div>
        ...
        // 5. MATCHING
            ...
            // Lấy tất cả đáp án đúng (Cột B) để làm options
            const columnB_Options = sec.questions.map(q => q.answer).sort(() => Math.random() - 0.5); 

            contentHtml = `
                <div class="bg-orange-50 p-4 rounded mb-4 text-orange-800 text-sm">
                    <i class="fa-solid fa-circle-info"></i> Chọn đáp án tương ứng từ danh sách thả xuống.
                </div>
                <div class="space-y-2">
                    ${sec.questions.map(q => {
...
// --- HÀM SỬA LỖI QUAN TRỌNG NHẤT Ở ĐÂY ---
function renderMultipleChoice(q, idx) {
    registerQuestion(q.id, idx);
    // Lỗi cũ: value="${opt}" (Sai, vì opt là nội dung chữ)
    // Sửa thành: value="${['A','B','C','D'][i]}" (Đúng, trả về A, B, C, D)
    return `
...
function startTimer(min) {
    ...
    timerInterval = setInterval(() => {
        ...
        if (time <= 300) display.classList.add('text-red-600', 'animate-pulse');
        if(time-- <= 0) { clearInterval(timerInterval); alert("Hết giờ làm bài!"); submitExam(true); }
    }, 1000);
}
...
window.submitExam = async function(force = false) {
    ...
    currentExam.sections.forEach(s => {
        if (s.type === 'writing') { writingPending = true; return; }
        s.questions.forEach(q => {
            total++;
            // So sánh đáp án (Logic quan trọng)
            if (userAnswers[q.id] && userAnswers[q.id].trim() === q.answer.trim()) correct++;
        });
    });
